...
def process_and_add_to_meilisearch():
    ...
    printline("Fetching unprocessed items from MongoDB...")
    ...
    for video_info in unprocessed_items:
        try:
            ...
            if video_index is not None:
                ...
                printline(f"process_and_add_to_meilisearch: Video Index: {video_index}")
            else:
                logging.info("process_and_add_to_meilisearch: Video Index is missing or null")
                printline("process_and_add_to_meilisearch: Video Index is missing or null")
            if video_id is not None:
                logging.info(f"process_and_add_to_meilisearch: Video Id: {video_id}")
                printline(f"process_and_add_to_meilisearch: Video Id: {video_id}")
            else:
                logging.info("process_and_add_to_meilisearch: Video Id is missing or null")
                printline("process_and_add_to_meilisearch: Video Id is missing or null")
        except KeyError as e:
            printline(f"Key error: {str(e)} - some required video information is missing")
            logging.info(f"Failed video_info: {video_info}")
        except Exception as e:
            printline(f"An error occurred: {str(e)}")
        ...
        if mresponse.status_code == 202:
            printline('Documents added successfully:', mresponse.json())
        else:
            printline('Failed to add documents', mresponse.content)
        ...
        printline(f"MeiliSearch Progress: {percentage_completed:.2f}% of {processed_count}/{total_items}")
        printline(f"Title: {doc['title']}, VideoURL: {doc['video_url']}")
...
def serve_hls(video_id, filename):
    ...
    if not os.path.exists(file_path):
        logging.info(f'Error serving HLS segment for video_id: {video_id}, filename: {filename}')
        logging.info(f"HLS: 16")
        abort(404, description="File not found")
    # Serve the requested file
    logging.info(f'Serving HLS segment for video_id: {video_id}, filename: {filename}')
    logging.info(f"HLS: 17")
    return send_from_directory(os.path.join(HLS_FILES_DIR, video_id), filename)
...
def video_page(video_id):
    ...
    if os.path.exists(hls_playlist_path):
        ...
        logging.info(f'HLS: 21 video_page app route: choosing video.html')
        ...
        logging.info(f"HLS: 22.0 Attempting to acquire lock for video_id {video_id}, {video['filename']}")
        with conversion_lock:
            logging.info(f"HLS: 22.0 Lock acquired for video_id {video_id}")
            if video_id not in ongoing_conversions:
                ongoing_conversions[video_id] = True
                try:
...
def download(url, args, cutout, output_base):
    ...
    try:
        ...
        printline('Line 1148: yt-dlp output is not valid JSON:', info_result)
